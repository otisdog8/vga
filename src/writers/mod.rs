...
pub trait GraphicsWriter<Color> {
    ...
    /// Draws a character at the given `(x, y)` coordinant to the specified `color` with a background color.
    ...
    fn draw_character_with_scaling(&self, x: usize, y: usize, scale: usize, character: char, color: Color, back_color: Color);
